...
class frmaOnayListe extends Component {
  constructor(props) {
    super(props);
    this.state = {}
  }
  async componentDidMount() {
    try {
      ...
      const response = await fetch(getIp() + '/react-native-insert/fOnayListe.php');
      ...
      Alert.alert("Onaylanan Firma Yok!");
  ...
  componentWillUnmount() {
    console.log("hatayı engellemek için bu sayfaya geri döndüğünde ne yapacağını bilsin diye sanıyorum:/")
  }
  render() {
    ...
    return (
      <>
        <LinearGradient colors={['#D9CAC1', '#A3B0D9']}
          ...
          <Text style={{ color: 'white', fontSize: 20, fontWeight: 'bold' }}>ONAYLANAN FİRMALAR</Text>
          {
            degisken2 == 'okulBasvuru' ?
              <FlatList
                style={styles.fl}
                ...
                <TouchableOpacity  onPress={() => Actions.jump('okulBasvuru', { firmaId: item.id, firmaAd: item.name })}>
              ...
              style={styles.fl}
                ...
                renderItem={({ item }) =>
                  <View style={{ justifyContent: 'center', alignItems: 'center' }}>
                    <TouchableOpacity onPress={() => Actions.frmaHocaOnay(
                      {
                        degisken1: item.name,
