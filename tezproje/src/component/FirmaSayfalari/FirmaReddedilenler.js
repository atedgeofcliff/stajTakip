...
class FirmaReddedilenler extends Component {
  ...
  async componentDidMount() {
    ...
    try {
      ...
      const response = await fetch(getIp() + '/react-native-insert/FirmaReddedilenler.php', {
        method: 'POST',
        headers: {
      ...
      if (responseJson == 'Başvuran Oğrenci Yok.') {
        Alert.alert("Reddedilen öğrenci yok");
  ...
  render() {
    ...
    return (
      ...
      <LinearGradient
        ...
        <Text style={{ color: 'white', fontSize: 20, fontWeight: 'bold' }}>REDDEDİLEN BEBELER</Text>
        ...
        <FlatList
          ...
          renderItem={({ item }) =>
            ...
            <View style={{ justifyContent: 'center', alignItems: 'center' }}>
              <TouchableOpacity onPress={() => Actions.FirmaReddedilenlerDetay(
                {
                  degisken: item.name,
...
export default FirmaReddedilenler;
