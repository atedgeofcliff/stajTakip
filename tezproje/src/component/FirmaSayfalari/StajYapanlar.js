...
class StajYapanlar extends Component {
  ...
  async componentDidMount() {
    ...
    try {
      ...
      const response = await fetch(getIp() + '/react-native-insert/firmaStajYapanlar.php', {
        method: 'POST',
        headers: {
      ...
      if (responseJson == 'Başvuran Oğrenci Yok.') {
        Alert.alert("Staj Yapan Öğrenci Yok.");
      ...
      <LinearGradient
        ...
        style={styles.fl}
          ...
          <View style={{  justifyContent: 'center', alignItems: 'center' }}>
              <TouchableOpacity o onPress={() => Actions.FirmaStajYapanlarDetay(
              {
                degisken: item.name,
                degisken1: item.email,
                degisken2: item.phone_number,
                comId: this.props.navigation.getParam('degisken1'),
                ogrenciId: item.id
              })}>
