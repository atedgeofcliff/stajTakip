...
class FirmaBasvuranlar extends Component {
  ...
  async componentDidMount() {
    ...
    try {
      ...
      const response = await fetch(getIp() + '/react-native-insert/firmaBasvuranlar.php', {
        method: 'POST',
        headers: {
        ...
        <FlatList
          ...
          renderItem={({ item }) =>
            ...
            <View style={{ justifyContent: 'center', alignItems: 'center' }}>
              <TouchableOpacity onPress={() => Actions.OgrenciDetayFirma(
                {
                  degisken: item.name,
...
export default FirmaBasvuranlar;
